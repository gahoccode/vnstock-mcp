...
async def calculate_returns(
    ...
    try:
        ...
        # Fetch and clean price data for multiple symbols concurrently
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                return f"Error fetching data for {symbols[i]}: {str(result)}"
            elif isinstance(result, str) and result.startswith("Error"):
                return result
            elif result is not None:
                all_data.append(result)
            else:
                return f"No data found for {symbols[i]} between {start_date} and {end_date}"
        ...
        combined_df = await loop.run_in_executor(
            None,
            lambda: pd.concat(all_data, axis=1)
        )
        ...
        mu = await loop.run_in_executor(
            None,
            lambda: expected_returns.mean_historical_return(
                combined_df,
                returns_data=False,
                compounding=False,
                frequency=252,
...
async def optimize_portfolio(
    ...
    try:
        ...
        def calculate_covariance(method):
            if method == "sample_cov":
                return risk_models.sample_cov(combined_df)
            elif method == "ledoit_wolf":
                return risk_models.ledoit_wolf_shrinkage(combined_df)
            elif method == "exp_cov":
                return risk_models.exp_cov(combined_df)
            elif method == "semicovariance":
                return risk_models.semicovariance(combined_df)
            else:
                return risk_models.sample_cov(combined_df)  # fallback to default

        S = await loop.run_in_executor(None, lambda: calculate_covariance(covariance_method))

        # Create Efficient Frontier object and run optimization in executor
        def run_optimization():
            ...
            return rounded_weights, performance

        clean_weights, performance = await loop.run_in_executor(None, run_optimization)
...
async def full_portfolio_optimization(
    ...
    try:
        ...
        # Run all three optimization types concurrently in executor
        def run_all_optimizations():
            ...
            try:
                ...
                performance_sharpe = ef_sharpe.portfolio_performance(verbose=False, risk_free_rate=risk_free_rate)
            ...
            try:
                ...
                performance_minvol = ef_minvol.portfolio_performance(verbose=False, risk_free_rate=risk_free_rate)
            ...
            try:
                ...
                performance_utility = ef_utility.portfolio_performance(verbose=False, risk_free_rate=risk_free_rate)
        ...
        optimizations = await loop.run_in_executor(None, run_all_optimizations)

        # Filter out failed optimizations for summary
        successful_opts = [k for k, v in optimizations.items() if v.get("success", False)]
