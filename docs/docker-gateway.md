...
ab3f5c7d9e2f   vnstock-mcp   "python server.py"  Up 1 second
...
Add to your Claude Code MCP settings (`.mcp.json` or project config):

**Simple (Docker MCP Toolkit manages catalogs):**
...
**Explicit (pin specific catalog, config, and registry files):**
...
> Use the explicit approach when you need to control exactly which catalogs are loaded or when the Docker MCP Toolkit auto-detection doesn't pick up your custom catalogs.
...
To add a new MCP server to the Docker gateway, you must update **3 files** — all using the **same server key**.

> **Critical:** The server key (e.g., `vnstock-mcp`) must be identical across all 3 files. A mismatch causes a silent "MCP server not found" failure.

| File | Purpose | Key location |
|------|---------|-------------|
| `docker/vnstock.yaml` | Server definition (image, tools, hosts) | `registry.<server-key>` |
| `docker/registry.yaml` | Registers the server with the gateway | `registry.<server-key>` |
| `docker/config.yaml` | Enables/disables the server | `servers.<server-key>` |

### Step 1: Add to catalog (`docker/vnstock.yaml`)
...
registry:
  my-new-server:
    ...
    title: "My New Server"
    type: server
    image: my-new-server:latest
    tools:
      - name: my_tool
        description: "What this tool does"
```

### Step 2: Register (`docker/registry.yaml`)
...
### Step 3: Enable (`docker/config.yaml`)
...
```bash
cp docker/vnstock.yaml ~/.docker/mcp/catalogs/vnstock.yaml
cp docker/registry.yaml ~/.docker/mcp/registry.yaml
cp docker/config.yaml ~/.docker/mcp/config.yaml
```

### Step 5: Restart Claude Desktop

Quit Claude completely (Cmd+Q) and relaunch.
...
- [NetworkChuck's Video Tutorial](https://www.youtube.com/watch?v=GuTcle5edjk)
